...
// src/services/api.js
// Add these new methods to the existing api.js file

export const visualizeBusbar = async (inputData) => {
    try {
        const response = await axios.post(`${API_URL}/busbar/visualize`, inputData);
        return response.data;
    } catch (error) {
        console.error('Error visualizing busbar:', error);
        throw error;
    }
};
...
export const generatePdfReport = async (projectId) => {
    try {
        // Using blob response type to handle PDF
        const response = await axios.get(`${API_URL}/report/generate/${projectId}`, {
            responseType: 'blob'
        });
...
export const generatePdfReportFromData = async (projectData) => {
    try {
        const response = await axios.post(`${API_URL}/report/generate`, projectData, {
            responseType: 'blob'
        });
